...
func clientPubrec(db *gorm.DB, p *packet.Packet, outQueue chan<- *out.OutData) {
	...
	onRetryFound := func(retry model.Retry) {
		...
		if retry.AckStatus == model.WAIT_FOR_PUB_REC {
			...
			log.Info().Msgf("pubrec for invalid retry status %s %s %s", retry.ClientId, retry.PacketIdentifier, retry.AckStatus)
		}
	}
	...
	if err := db.First(&retry).Error; err != nil {
		log.Info().Msgf("pubrec for invalid retry %s %s", retry.ClientId, retry.PacketIdentifier)
	} else {
		onRetryFound(retry)
	}
}
